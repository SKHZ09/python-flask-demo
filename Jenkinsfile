pipeline {
   ...
   agent any //{label 'slave_linux'}
    ...
    //stage('Checkout SCM')   {
      ...
      //  steps {
      //git ('https://github.com/arksinha93/python-flask-demo.git%27)
    ...
    stage('Test') {
       ...
       stage('Sast') {
            steps {
               ...
               post {
                failure {
                   mail to: 'saikanth.pantangi@gmail.com', subject: 'Compile failed', body: 'Please fix the build'
                   //Jira newIssue: true, summary: 'Compile failed', description: 'Please fix the build'
                         }
                success {
                   mail to: 'saikanth.pantangi@gmail.com', subject: 'Unit tests passed', body: 'Congratulations!'
                        }
                     }
        }
    stage('Dast') {
